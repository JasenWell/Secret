...
import com.xyp.mimi.im.im.message.GroupClearMessage;
import com.xyp.mimi.im.im.message.PokeMessage;
import com.xyp.mimi.im.im.message.SealContactNotificationMessage;
import com.xyp.mimi.im.im.message.SealGroupConNtfMessage;
import com.xyp.mimi.im.im.message.SealGroupNotificationMessage;
import com.xyp.mimi.im.im.provider.ContactNotificationMessageProvider;
import com.xyp.mimi.im.im.provider.GroupApplyMessageProvider;
import com.xyp.mimi.im.im.provider.SealGroupConNtfMessageProvider;
import com.xyp.mimi.im.im.provider.SealGroupNotificationMessageItemProvider;
import com.xyp.mimi.im.model.ChatRoomAction;
import com.xyp.mimi.im.model.ContactNotificationMessageData;
import com.xyp.mimi.im.model.ConversationRecord;
import com.xyp.mimi.im.model.LoginResult;
import com.xyp.mimi.im.model.QuietHours;
import com.xyp.mimi.im.model.Resource;
import com.xyp.mimi.im.model.UserCacheInfo;
import com.xyp.mimi.im.net.CallBackWrapper;
import com.xyp.mimi.im.net.HttpClientManager;
import com.xyp.mimi.im.net.service.UserService;
import com.xyp.mimi.im.sp.UserCache;
import com.xyp.mimi.im.sp.UserConfigCache;
import com.xyp.mimi.im.ui.activity.ConversationActivity;
import com.xyp.mimi.im.ui.activity.GroupNoticeListActivity;
import com.xyp.mimi.im.ui.activity.NewFriendListActivity;
import com.xyp.mimi.im.ui.activity.PokeInviteChatActivity;
                ...
                if (messageContent instanceof ContactNotificationMessage) { // 添加好友状态信息
                    ...
                    if (getReceivePokeMessageStatus()) {
                        ...
                        if (!isInConversation) {
                             ...
                             */
                            if (BaseApp.getApplication().isAppInForeground()
                                    && BaseApp.getApplication().isMainActivityCreated()) {
                                context.startActivity(showPokeIntent);
                            } else {
                                // 若之前有未启动的戳一下消息则默认启动第一个戳一下消息
                                Intent lastIntent = BaseApp.getApplication().getLastOnAppForegroundStartIntent();
                                if (lastIntent == null
                                        || (lastIntent.getComponent() != null
                                        && !lastIntent.getComponent().getClassName().equals(PokeInviteChatActivity.class.getName()))) {
                                    BaseApp.getApplication().setOnAppForegroundStartIntent(showPokeIntent);
                                }
                            }
                        }
                        return false;
