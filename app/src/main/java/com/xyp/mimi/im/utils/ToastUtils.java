...
public class ToastUtils {
    ...
    public static void showErrorToast(ErrorCode errorCode) {
        ...
        if (lastToast != null) {
            ...
            lastToast = Toast.makeText(BaseApp.getInstance(), message, Toast.LENGTH_SHORT);
        }
        lastToast.show();
    }
    ...
    public static void showToast(int resourceId, int duration) {
        showToast(BaseApp.getInstance().getResources().getString(resourceId), duration);
    }
    ...
    public static void showToast(String message, int duration) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P){
             Toast.makeText(BaseApp.getInstance(), message, duration).show();
        } else {
            if (lastToast != null) {
                lastToast.setText(message);
            } else {
                lastToast = Toast.makeText(BaseApp.getInstance(), message, duration);
            }
            lastToast.show();
        }
    }
